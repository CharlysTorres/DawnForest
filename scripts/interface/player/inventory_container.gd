...
func _ready() -> void:
	for icon in aux_h_container.get_children():
		icon.connect("mouse_exited", self, "mouse_interection", ["exited", icon])
		icon.connect("mouse_entered", self, "mouse_interection", ["entered", icon])
...
func update_slot(item_name: String, item_image: StreamTexture, item_info: Array) -> void:
	...
	if existing_item_index != -1:
		...
		if item_slot.amount < 25 and item_slot.item_type != "Equipment" and item_slot.item_type != "Weapon":
			var current_amount: int = item_slot.amount + item_info[4]
			if current_amount > 25:
				var leftover: int = current_amount - 25
				item_info[3] = 25 - item_slot.amount
...
func mouse_interection(state: String, object: TextureRect) -> void:
	match state:
		"entered":
			can_click = true
...
func _process(_delta: float) -> void:
	if Input.is_action_just_pressed("player1_select_item") and can_click and current_state != "":
		match current_state:
			"Equip":
